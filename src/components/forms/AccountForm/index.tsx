...
export const AccountForm: React.FC<{ serverUser?: User }> = ({ serverUser }) => {
  ...
  const onSubmit = useCallback(
    async (data: FormData) => {
      ...
      if (currentUser) {
        const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/users/${currentUser.id}`, {
    ...
    [user, serverUser, setUser, reset],
  )

  useEffect(() => {
    // Use server user data if available, otherwise fall back to auth context
    const currentUser = serverUser || user

    if (user === null && !serverUser) {
      router.push(
  ...
  }, [user, serverUser, router, reset, changePassword])
